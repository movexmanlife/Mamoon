 ...
 *
 */
public class ArcRateChart extends View {
    ...
    public ArcRateChart(Context context) {
        this(context, null);
    }

    public ArcRateChart(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ArcRateChart(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOriginTotalRateSum == 0) {
            if (mNoRatePaint == null) {
                mNoRatePaint = new Paint();
                setPaintAttrs(mNoRatePaint, mNoRateColor, Style.FILL);
            }
            if (mNoRateOval == null) {
                mNoRateOval = new RectF(left, top, right, bottom);
            }
            canvas.drawArc(mNoRateOval, 0, progress, true, mNoRatePaint);
            canvas.drawCircle(centerX, centerY, bgCircleRadius, mBgCirclePaint);
            return;
        }

        // 绘制大圆弧，填充圆弧带圆心（扇形），大圆弧不断增长
        if (mBigOval == null) {
    ...
    /**
     * 设置动画效果
     * @param smallArc 小圆弧（比率，圆弧颜色）
     * @param bigArc 大圆弧（比率，圆弧颜色）
     */
    public void startAnimation(ArcInfo smallArc, ArcInfo bigArc) {
        if (isAnimating) {
            return;
        }

        if (smallArc.mRate < 0 || bigArc.mRate < 0) {
            throw new IllegalArgumentException(
                    "Params smallRate or bigRate must be >=0.");
        }
        initRateValueAndColor(smallArc, bigArc);
        calcAngles();
    ...
    /**
     * 初始化"比率值"，及不同"比率"对应的颜色
     *
     * @param smallArc
     * @param bigArc
     */
    private void initRateValueAndColor(ArcInfo smallArc, ArcInfo bigArc) {
        if (smallArc.mRate > bigArc.mRate) {
            throw new IllegalArgumentException(
                    "Params smallRate cannot greater than bigRate.");
        }
        mOriginSmallRate = smallArc.mRate;
        mOriginBigRate = bigArc.mRate;
        mOriginTotalRateSum = mOriginBigRate + mOriginSmallRate;

        mSmallRateColor = mContext.getResources().getColor(smallArc.mColorId);
        mBigRateColor = mContext.getResources().getColor(bigArc.mColorId);
    }
    ...
    private void calcAngles() {
        ...
        if (mOriginSmallRate == 0) {
            mSmallRateAngle = 0;
            mBigRateAngle = CIRCLE_ANGLE;
            mRotateAngle = 0;
            return;
        }
        ...
        double smallValue = (mOriginSmallRate / mOriginTotalRateSum) * realTotalAngle;
        if (smallValue < 1) { // 值小于1，则当做1处理
            mSmallRateAngle = 1;
        } else {
            mSmallRateAngle = (int) smallValue;
        }

        mBigRateAngle = realTotalAngle - mSmallRateAngle;
        mRotateAngle = ((mBigRateAngle - CIRCLE_ANGLE / 2) * (-1)) / 2; // 逆时针旋转
    }
